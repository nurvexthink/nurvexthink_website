...
% ---------- hyperref ----------
\hypersetup{
  pdftitle={NurvexThink -- Products Admin Tiered Editor (PR 3 of 3) -- Implementation Plan},
...
\rhead{\small\color{slate}Products Admin Plan}
...
\vspace{1.6cm}
{\Large\bfseries\color{navy} Products Admin Tiered Editor (PR 3 of 3) --- Implementation Plan}\\[0.4cm]
{\large\color{slate} Quick View / Features / Technical / SEO \& Links tabs, publish validation, drag reorder \& categories, and a real-component draft preview --- on the PR 1 schema and PR 2 public UX}

\vfill
\begin{tabular}{rl}
\textbf{Date:} & 2026-07-05\\
\textbf{Status:} & Implementation plan (PR 3 of 3)\\
\textbf{Owner:} & Fatima Abdul Raheem (CEO)\\
\textbf{Branch:} & \texttt{feat/products-admin}\\
\end{tabular}
\vspace{1.5cm}
\end{titlepage}
...
The complete products admin on top of the PR 1 schema and PR 2 public UX --- a tiered editor (Quick
View / Features / Technical / SEO \& Links tabs, every field labeled with its public destination),
publish validation that also guards edits to already-published rows (with the ``Unpublish \& save as
draft'' escape), image uploads to the \texttt{product-images} bucket, a drag-reorderable + filterable
list with Duplicate, an admin-managed categories page with a delete guard, and a preview route that
renders the \textbf{actual} public components from the last-saved draft.

% ===================================================================
\section{Architecture}

The editor is one client island holding a single form object (plus keyed lists for drag-and-drop) and
submits a \textbf{JSON payload} to one server action, \texttt{saveProductFull} --- nested lists
(highlights, features, related-post links) don't fit flat FormData, and one action keeps the product +
child-table writes together. Child rows (\texttt{product\_features}, \texttt{product\_blog\_links}) are
replaced wholesale on save (delete + insert; volumes are tiny, ordering writes become trivial). Publish
validation is a \textbf{pure helper} used twice: client-side for instant feedback (Publish button state +
checklist) and server-side as the authority. Drag-and-drop is three shared primitives
(\texttt{SortableList}/\texttt{SortableItem} over @dnd-kit) reused by five surfaces: highlights, features,
related posts, the product list, and the categories manager. The preview route fetches with the admin
server client, maps through the \textbf{same \texttt{toProductDetail}} the public routes use, and renders
the untouched \texttt{ProductDetailView} / \texttt{ProductQuickView} --- zero changes to public
components. Uploads go through a server action (admin session + storage RLS), which requires bumping
Next's server-action body limit above the default 1\,MB.

\textbf{Tech Stack:} Next.js 16 App Router, React 19, Tailwind v4, Supabase (server client as the
signed-in admin; RLS is the write boundary), @dnd-kit, Vitest + Testing Library.
...
\begin{itemize}
  \item Brand: \textbf{NurvexThink} / \texttt{nurvexthink}. Conventional Commits. Branch:
        \texttt{feat/products-admin} (from current \texttt{main}).
  \item \textbf{The ONLY new dependencies are \texttt{@dnd-kit/core}, \texttt{@dnd-kit/sortable},
        \texttt{@dnd-kit/utilities}} (spec \S6 sanctions exactly this: accessible keyboard + touch DnD).
  \item \textbf{No changes to public components or queries} --- \texttt{product-quick-view.tsx},
        \texttt{product-detail-view.tsx}, \texttt{product-card.tsx}, \texttt{products-explorer.tsx}, and
        everything in \texttt{src/lib/queries.ts} are imported, never edited.
  \item Admin auth: the \texttt{(panel)} layout already redirects signed-out users
        (\texttt{src/app/admin/(panel)/layout.tsx}); server actions run as the signed-in admin via
        \texttt{createServerSupabaseClient()} and \textbf{RLS is the write boundary}
        (\texttt{private.is\_admin()} policies from 0001/0003). \texttt{service\_role} is never used.
  \item \textbf{Every mutation revalidates} \texttt{/}, \texttt{/products}, the affected
        \texttt{/products/[slug]} path(s), and \texttt{/admin/products} (the established
        \texttt{revalidateAll} pattern, extended).
  \item Spec contract: \texttt{docs/superpowers/specs/2026-07-04-products-experience-design.md}
        \S3 (Admin UX), \S8.3.
  \item Publish validation (verbatim from spec \S3): the Quick View tier --- \textbf{name, tagline,
        summary, $\geq$ 3 highlights, cover image, category} --- must be complete for a product to be
        \textbf{or remain} published. Publish enables only when valid; saving edits to a published
        product re-runs the same validation and an invalid save is blocked with the \textit{Unpublish \&
        save as draft} option; drafts save anytime, half-finished.
  \item Duplicate semantics (verbatim from spec \S3): new \textbf{draft}; slug \texttt{-copy} /
        \texttt{-copy-2} with a uniqueness retry; \texttt{featured} reset to false; \texttt{sort\_order} =
        end of list; features + related-post links copied; \textbf{image URLs copied by reference}
        (shared Storage objects --- future delete-cleanup must check referrers; already flagged in the
        spec).
  \item Reordering is \textbf{disabled while any filter is active} (positions are global) and the list
        groups featured items first to mirror the public order.
  \item Uploads: \texttt{product-images} bucket, content-type \texttt{image/*}, \textbf{$\leq$ 5\,MB}, via
        a server action (needs the \texttt{serverActions.bodySizeLimit} bump in \texttt{next.config.ts}
        --- Next's default is 1\,MB).
  \item Server Components by default; \texttt{"use client"} only at leaves: the editor, the list table,
        the categories manager, upload fields, sortable primitives, and the preview quick-view wrapper
        (CLAUDE.md).
  \item Definition of green: \texttt{npm run typecheck \&\& npm run lint \&\& npm test \&\& npm run
        build} all exit 0 --- \textbf{after every task}.
\end{itemize}
...
\begin{itemize}
  \item \texttt{next.config.ts} --- \textbf{modify}: \texttt{serverActions.bodySizeLimit: "6mb"} so
        $\leq$ 5\,MB image uploads survive multipart overhead.
  \item \texttt{src/lib/product-admin.ts} --- \textbf{create}: pure helpers --- publish-tier validation,
        \texttt{slugify}, duplicate-slug candidates, sort-order math. \texttt{src/lib/product-admin.test.ts}
        --- \textbf{create} (TDD).
  \item \texttt{src/lib/admin-queries.ts} --- \textbf{modify} (full rewrite): public-mirroring product
        order + category join, \texttt{getProductFullAdmin} (features + links + post status),
        \texttt{listCategoriesAdmin}, \texttt{countProductsByCategory}, \texttt{listPostsForPicker};
        blog/orders helpers unchanged.
  \item \texttt{src/app/admin/(panel)/products/actions.ts} --- \textbf{modify} (full rewrite):
        \texttt{saveProductFull} (JSON payload, tier validation, child-row replace),
        \texttt{reorderProducts}, \texttt{duplicateProduct}, \texttt{uploadProductImage},
        \texttt{deleteProduct}; legacy form actions kept temporarily so old consumers compile, removed in
        Tasks 2--3.
  \item \texttt{src/app/admin/(panel)/products/categories/actions.ts} --- \textbf{create}: category CRUD
        + reorder with the referenced-delete guard.
  \item \texttt{src/components/admin/form-styles.ts} --- \textbf{create}: the shared
        \texttt{fieldClass}/\texttt{labelClass}/\texttt{hintClass} strings (previously private to
        \texttt{product-form.tsx}).
  \item \texttt{src/components/admin/sortable-list.tsx} --- \textbf{create} (\texttt{"use client"}):
        \texttt{SortableList} + \texttt{SortableItem} @dnd-kit primitives.
  \item \texttt{src/components/admin/image-upload.tsx} --- \textbf{create} (\texttt{"use client"}):
        \texttt{ImageUploadField} (single, with preview/replace/remove) + \texttt{GalleryUploadField}
        (multi with remove).
  \item \texttt{src/components/admin/related-posts-picker.tsx} --- \textbf{create}
        (\texttt{"use client"}): search-filter over all posts, multi-select, drag-order, draft badge.
  \item \texttt{src/components/admin/product-editor.tsx} --- \textbf{create} (\texttt{"use client"}): the
        tiered editor. \texttt{src/components/admin/product-editor.test.tsx} --- \textbf{create}.
  \item \texttt{src/components/admin/products-table.tsx} --- \textbf{create} (\texttt{"use client"}):
        list rows, filters, grouped drag reorder, Duplicate.
  \item \texttt{src/components/admin/categories-manager.tsx} --- \textbf{create} (\texttt{"use client"}):
        add / rename / delete-guarded / reorder.
  \item \texttt{src/components/admin/preview-quick-view.tsx} --- \textbf{create} (\texttt{"use client"}):
        mounts \texttt{ProductQuickView} open, \texttt{onClose} $\rightarrow$ \texttt{router.back()}.
  \item \texttt{src/app/admin/(panel)/products/page.tsx} --- \textbf{modify} (rewrite): renders
        \texttt{ProductsTable}.
  \item \texttt{src/app/admin/(panel)/products/new/page.tsx}, \texttt{.../[id]/page.tsx} ---
        \textbf{modify} (rewrite): render \texttt{ProductEditor} with categories + posts.
  \item \texttt{src/app/admin/(panel)/products/categories/page.tsx} --- \textbf{create}: categories
        manager route.
  \item \texttt{src/app/admin/(panel)/products/[id]/preview/page.tsx} --- \textbf{create}: both-tier
        preview from the last-saved draft.
  \item \texttt{src/components/admin/product-form.tsx} --- \textbf{delete} (Task 2; replaced by the
        tiered editor).
\end{itemize}

% ===================================================================
\section{Task 1: Data layer + server actions (TDD on the pure logic)}

\subsection*{Files}
\begin{itemize}
  \item Create: \texttt{src/lib/product-admin.ts}, \texttt{src/lib/product-admin.test.ts}
  \item Modify: \texttt{src/lib/admin-queries.ts} (full-file replace)
  \item Modify: \texttt{src/app/admin/(panel)/products/actions.ts} (full-file replace)
  \item Create: \texttt{src/app/admin/(panel)/products/categories/actions.ts}
  \item Modify: \texttt{next.config.ts}
\end{itemize}

\subsection*{Interfaces}
\begin{itemize}
  \item Consumes: \texttt{ProductRow}, \texttt{ProductCategoryRow}, \texttt{BlogPostRow} from
        \texttt{src/lib/supabase/types.ts}; \texttt{createServerSupabaseClient} from
        \texttt{src/lib/supabase/server.ts}.
  \item Produces (later tasks rely on these exact names):
  \begin{itemize}
    \item \texttt{src/lib/product-admin.ts}:
          \lstinline!validatePublishTier(f: PublishTierFields): { valid: boolean; missing: string[] }!,
          \lstinline!slugify(text)!, \lstinline!copySlug(slug, attempt)!,
          \lstinline!nextSortOrder(existing: number[])!, \lstinline!sortOrderSequence(count)!.
    \item \texttt{src/lib/admin-queries.ts}: types \texttt{AdminProductRow}, \texttt{AdminFeatureJoin},
          \texttt{AdminLinkJoin}, \texttt{AdminProductFull}, \texttt{PostPickerItem}; functions
          \lstinline!listProductsAdmin()! (ordered \texttt{featured DESC, sort\_order ASC, created\_at
          DESC} --- mirrors public), \lstinline!getProductFullAdmin(id)!, \lstinline!listCategoriesAdmin()!,
          \lstinline!countProductsByCategory()!, \lstinline!listPostsForPicker()!.
    \item \texttt{actions.ts}: types \texttt{ProductPayload}, \texttt{EditorFeature},
          \lstinline!SaveIntent = "save" | "publish" | "unpublish"!, \texttt{SaveProductResult},
          \texttt{MutationResult}, \texttt{UploadResult}; actions
          \lstinline!saveProductFull(id | null, intent, payload)!, \lstinline!reorderProducts(orderedIds)!,
          \lstinline!duplicateProduct(id)!, \lstinline!uploadProductImage(formData)!,
          \lstinline!deleteProduct(formData)!.
    \item \texttt{categories/actions.ts}: \lstinline!createCategory(name)!,
          \lstinline!renameCategory(id, name)!, \lstinline!deleteCategory(id)!,
          \lstinline!reorderCategories(orderedIds)! --- all return
          \lstinline!CategoryActionResult = { ok: boolean; error: string | null }!.
  \end{itemize}
\end{itemize}

\subsection*{Step 1: Branch}

\begin{lstlisting}
git checkout main && git pull && git checkout -b feat/products-admin
\end{lstlisting}

\subsection*{Step 2: Write failing tests}

Create \texttt{src/lib/product-admin.test.ts}:
...
\subsection*{Step 3: Run to verify failure}

Run: \texttt{npx vitest run src/lib/product-admin.test.ts} --- expect FAIL
(\texttt{Cannot find module '@/lib/product-admin'}).

\subsection*{Step 4: Create \texttt{src/lib/product-admin.ts}}
...
\subsection*{Step 5: Run tests --- pass}

Run: \texttt{npx vitest run src/lib/product-admin.test.ts} $\rightarrow$ all green (\textbf{10 tests}).

\subsection*{Step 6: Replace \texttt{src/lib/admin-queries.ts}}

Whole file --- blog/orders helpers reproduced unchanged; \texttt{getProductAdmin} kept as legacy until
Task 2 rewrites the edit page:
...
\subsection*{Step 7: Replace \texttt{src/app/admin/(panel)/products/actions.ts}}

Whole file. The legacy block at the bottom keeps the old form + list compiling until Tasks 2--3 replace
them:
...
\subsection*{Step 8: Create \texttt{src/app/admin/(panel)/products/categories/actions.ts}}
...
\subsection*{Step 9: Bump the server-action body limit}

Replace \texttt{next.config.ts} with:
...
If \texttt{next build} in Step 10 warns that \texttt{experimental.serverActions} is unrecognized (Next
16 may have promoted it), move the \texttt{serverActions} block to the config top level and re-run ---
the green bar decides.

\subsection*{Step 10: Green bar}

Run: \texttt{npm run typecheck \&\& npm run lint \&\& npm test \&\& npm run build} $\rightarrow$ all
green (build proves the old list/edit pages still compile against the legacy exports).
...
% ===================================================================
\section{Task 2: Tiered editor UI (Quick View / Features / Technical / SEO \& Links)}

\subsection*{Files}
\begin{itemize}
  \item Modify: \texttt{package.json} (via \texttt{npm install} --- the one sanctioned dependency
        addition)
  \item Create: \texttt{src/components/admin/form-styles.ts}
  \item Create: \texttt{src/components/admin/sortable-list.tsx}
  \item Create: \texttt{src/components/admin/image-upload.tsx}
  \item Create: \texttt{src/components/admin/related-posts-picker.tsx}
  \item Create: \texttt{src/components/admin/product-editor.tsx},
        \texttt{src/components/admin/product-editor.test.tsx}
  \item Modify: \texttt{src/app/admin/(panel)/products/new/page.tsx},
        \texttt{src/app/admin/(panel)/products/[id]/page.tsx} (whole-file replaces)
  \item Delete: \texttt{src/components/admin/product-form.tsx}
  \item Modify: \texttt{src/app/admin/(panel)/products/actions.ts} (remove legacy block A),
        \texttt{src/lib/admin-queries.ts} (remove \texttt{getProductAdmin})
\end{itemize}

\subsection*{Interfaces}
\begin{itemize}
  \item Consumes: everything Task 1 produced; \texttt{ProductCategoryRow} from types;
        \texttt{buttonVariants}, \texttt{cn}.
  \item Produces:
        \lstinline!<ProductEditor product={AdminProductFull | null} categories={ProductCategoryRow[]} posts={PostPickerItem[]} />!;
        \lstinline!<SortableList ids onReorder disabled?>! +
        \lstinline!<SortableItem id disabled? handleTitle? className?>! (Task 3 reuses both);
        \lstinline!<ImageUploadField label destination value onChange />!,
        \lstinline!<GalleryUploadField value onChange />!;
        \lstinline!<RelatedPostsPicker posts selectedIds onChange />!;
        \texttt{fieldClass}/\texttt{labelClass}/\texttt{hintClass} from \texttt{form-styles}.
\end{itemize}

\subsection*{Step 1: Install @dnd-kit}

THE one new dependency of this feature, spec \S6:

\begin{lstlisting}
npm install @dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities
\end{lstlisting}

Expected: \texttt{package.json} dependencies now include \texttt{@dnd-kit/core},
\texttt{@dnd-kit/sortable}, \texttt{@dnd-kit/utilities}; lockfile updated; no other dependency changes.

\subsection*{Step 2: Create \texttt{src/components/admin/form-styles.ts}}

The strings previously private to \texttt{product-form.tsx}, now shared by editor/picker/table/manager:
...
\subsection*{Step 3: Create \texttt{src/components/admin/sortable-list.tsx}}
...
\subsection*{Step 4: Create \texttt{src/components/admin/image-upload.tsx}}
...
\subsection*{Step 5: Create \texttt{src/components/admin/related-posts-picker.tsx}}
...
\subsection*{Step 6: Create \texttt{src/components/admin/product-editor.tsx}}

The tiered editor. One client island; all state lifted here (tabs render conditionally without losing
anything); submits the Task 1 action with a JSON payload:
...
\subsection*{Step 7: Rewrite the new/edit pages}

Replace \texttt{src/app/admin/(panel)/products/new/page.tsx} with:
...
Replace \texttt{src/app/admin/(panel)/products/[id]/page.tsx} with:
...
\subsection*{Step 8: Delete the old form and legacy block A}

\begin{lstlisting}
git rm src/components/admin/product-form.tsx
\end{lstlisting}

In \texttt{src/app/admin/(panel)/products/actions.ts}, delete \texttt{ProductFormState},
\texttt{parseForm}, \texttt{createProduct}, and \texttt{updateProduct} (everything from
\texttt{export type ProductFormState} through the end of \texttt{updateProduct} --- keep
\texttt{toggleProductPublished}, which the list page still imports until Task 3), and remove the
now-unused \texttt{redirect} import line (\texttt{import \{ redirect \} from "next/navigation";}). Update
the legacy comment block to read:
...
In \texttt{src/lib/admin-queries.ts}, delete the legacy \texttt{getProductAdmin} function (the
three-line function plus its \texttt{// Legacy --- removed in Task 2\ldots{}} comment). \texttt{ProductRow} stays
imported (\texttt{AdminProductRow} extends it).

\subsection*{Step 9: Editor component test}

Create \texttt{src/components/admin/product-editor.test.tsx}:
...
\subsection*{Step 10: Run the new tests}

Run: \texttt{npx vitest run src/components/admin/product-editor.test.tsx} $\rightarrow$ PASS (4 tests).

\subsection*{Step 11: Green bar}
...
% ===================================================================
\section{Task 3: List view + categories manager + preview route}

\subsection*{Files}
\begin{itemize}
  \item Create: \texttt{src/components/admin/products-table.tsx}
  \item Modify: \texttt{src/app/admin/(panel)/products/page.tsx} (whole-file replace)
  \item Modify: \texttt{src/app/admin/(panel)/products/actions.ts} (remove
        \texttt{toggleProductPublished})
  \item Create: \texttt{src/app/admin/(panel)/products/categories/page.tsx},
        \texttt{src/components/admin/categories-manager.tsx}
  \item Create: \texttt{src/app/admin/(panel)/products/[id]/preview/page.tsx},
        \texttt{src/components/admin/preview-quick-view.tsx}
\end{itemize}

\subsection*{Interfaces}
\begin{itemize}
  \item Consumes: \texttt{listProductsAdmin}, \texttt{listCategoriesAdmin},
        \texttt{countProductsByCategory}, \texttt{getProductFullAdmin} (Task 1);
        \texttt{reorderProducts}, \texttt{duplicateProduct}, \texttt{deleteProduct}, category actions
        (Task 1); \texttt{SortableList}/\texttt{SortableItem}, \texttt{form-styles} (Task 2);
        \textbf{unchanged public exports} \texttt{toProductDetail} (\texttt{src/lib/queries.ts}),
        \texttt{ProductDetailView}, \texttt{ProductQuickView} (\texttt{ProductQuickView} needs
        \texttt{product}/\texttt{open}/\texttt{onClose} --- the small client wrapper supplies them).
  \item Produces: \lstinline!<ProductsTable products categories />!;
        \lstinline!<CategoriesManager categories={CategoryWithCount[]} />!;
        \lstinline!<PreviewQuickView product={Product} />!; routes
        \texttt{/admin/products/categories} and \texttt{/admin/products/[id]/preview} (+
        \texttt{?tier=quick}).
\end{itemize}

\subsection*{Step 1: Create \texttt{src/components/admin/products-table.tsx}}
...
\subsection*{Step 2: Replace \texttt{src/app/admin/(panel)/products/page.tsx}}
...
\subsection*{Step 3: Remove the last legacy action}

In \texttt{src/app/admin/(panel)/products/actions.ts}, delete the legacy comment block and the whole
\texttt{toggleProductPublished} function (publishing now only flows through \texttt{saveProductFull}'s
validated intents). Nothing imports it anymore --- \texttt{npm run typecheck} confirms.

\subsection*{Step 4: Create \texttt{src/components/admin/categories-manager.tsx}}
...
\subsection*{Step 5: Create \texttt{src/app/admin/(panel)/products/categories/page.tsx}}
...
\subsection*{Step 6: Create \texttt{src/components/admin/preview-quick-view.tsx}}
...
\subsection*{Step 7: Create \texttt{src/app/admin/(panel)/products/[id]/preview/page.tsx}}
...
Note: \texttt{ProductDetail} extends \texttt{Product}, so \texttt{detail} feeds both components;
\texttt{toProduct} (inside \texttt{toProductDetail}) builds the quick view's \texttt{related} chips from
the same filtered links. \texttt{AdminProductFull} is structurally a \texttt{ProductRowJoined} (its
\texttt{AdminLinkJoin.blog\_posts} carries an extra \texttt{status} field, which the mapper ignores).

\subsection*{Step 8: Green bar}

Run: \texttt{npm run typecheck \&\& npm run lint \&\& npm test \&\& npm run build} $\rightarrow$ all
green. Build output lists the two new routes: \texttt{/admin/products/categories} and
\texttt{/admin/products/[id]/preview}.

\subsection*{Step 9: Commit}
...
% ===================================================================
\section{Task 4: Visual QA, plan PDF twin, PR}

\textit{(Controller-level task --- needs the local server, admin credentials, and human-visible
artifacts.)}

\subsection*{Step 1: Boot}

\texttt{npm run build} green $\rightarrow$ \texttt{npm run dev} $\rightarrow$ sign in at
\texttt{/admin/login}.

\subsection*{Step 2: Validation walkthrough (the spec \S3 contract, end to end)}

\begin{enumerate}
  \item \textbf{Create draft:} New product $\rightarrow$ type a name, watch the slug auto-fill; edit the
        slug once and confirm it stops following the name. Add only \textbf{2} highlights $\rightarrow$
        \textbf{Publish is disabled} and the footer lists the missing fields (must include ``At least 3
        highlights'', ``Cover image'', ``Category'').
  \item \textbf{Fill:} third highlight, tagline, summary, upload a cover ($<$ 5\,MB), pick a category
        $\rightarrow$ Publish enables. Click \textbf{Save draft} (lands on the product's editor).
  \item \textbf{Preview both tiers:} Preview $\rightarrow$ technical page renders with feature blocks in
        editor order; switch to \textbf{Quick View} $\rightarrow$ the real dialog opens; \textbf{Esc
        returns to the tier-2 preview} (history.back).
  \item \textbf{Publish:} Publish $\rightarrow$ \texttt{/products} shows the card immediately
        (revalidatePath, no redeploy) and \texttt{/products/[slug]} renders.
  \item \textbf{Guarded edit:} reopen the published product, \textbf{remove the cover image}
        $\rightarrow$ Save changes $\rightarrow$ blocked banner names ``Cover image'' and offers
        \textbf{Unpublish \& save as draft instead} $\rightarrow$ click it $\rightarrow$ product is
        Draft, the public detail page 404s and the card is gone.
  \item \textbf{Duplicate:} Duplicate a product $\rightarrow$ new Draft row ``\ldots{} (copy)'' with slug
        \texttt{\ldots{}-copy} at the \textbf{end} of the list, not featured, features + related links
        present in its editor; duplicate the same product again $\rightarrow$ \texttt{\ldots{}-copy-2}.
  \item \textbf{Reorder:} drag a row $\rightarrow$ \texttt{/products} order follows (featured block stays
        first). Set any filter $\rightarrow$ drag handles disable and the ``positions are global'' hint
        appears.
  \item \textbf{Categories:} add one $\rightarrow$ it appears in the editor select + as a pill on
        \texttt{/products} once a published product uses it; rename it; try deleting one \textbf{in use}
        $\rightarrow$ guarded error tells you to reassign; reassign the product, delete succeeds;
        drag-reorder categories $\rightarrow$ pill order on \texttt{/products} follows.
  \item \textbf{Upload guards:} try a $>$ 5\,MB image and a \texttt{.txt} file $\rightarrow$ inline
        errors, nothing uploaded.
\end{enumerate}

\subsection*{Step 3: Screenshots}

(dark + light): admin list (featured group + filters), each editor tab, the blocked-save banner with the
unpublish escape, preview both tiers, categories page. Attach to the PR.

\subsection*{Step 4: Plan PDF twin}

Generate \texttt{docs/superpowers/plans/2026-07-05-products-admin.tex} + \texttt{.pdf} (brand LaTeX
template, lstlistings for code) and commit (\texttt{docs:} prefix).

\subsection*{Step 5: PR}

Push \texttt{feat/products-admin}; open a PR titled ``feat: products admin --- tiered editor, drag
reorder, categories, preview'' summarizing spec \S3 coverage, the QA checklist results, and screenshots.
\texttt{main} is PR-protected --- 1 review required.

\subsection*{Step 6: After merge + deploy}

Repeat steps 2.4 and 2.9 against production (Vercel + Supabase Cloud); confirm uploads land in the
\texttt{product-images} bucket.

% ===================================================================
\section{Self-Review (done)}
\begin{itemize}
  \item \textbf{Spec \S3 coverage:} list view --- cover thumb / status badge / lifecycle / featured star
        (done), status + category filters (done), drag-to-reorder persisting \texttt{sort\_order} (done),
        disabled while filtered with the ``positions are global'' explanation (done), featured grouped
        first with drags never crossing the boundary (done), Duplicate (done). Editor --- four tabs
        mirroring the public tiers (done); every field labeled with its public destination (the research
        pattern) (done); auto-slug with manual override (done); highlights add/remove/drag with the
        ``plain language, no jargon'' helper (done); category select + Manage categories link (done);
        cover upload with preview (done); features repeating rows (image/title/explanation, add/remove/
        drag, optional-at-publish) (done); technical tab (description, technical\_details, tech list,
        gallery multi-upload with remove, repo URL, year) (done); SEO \& Links (seo\_description
        ``(search results)'', og\_image, related-posts picker with search/multi-select/drag-order/draft
        badge) (done). Publish validation on publish \textbf{and} on saves of published rows, with the
        Unpublish \& save as draft escape, errors naming the missing quick-view fields (done) (helper
        unit-tested; enforced server-side; mirrored client-side for the disabled Publish button). Preview
        renders the actual \texttt{ProductQuickView} + \texttt{ProductDetailView} from the last-saved
        draft via admin fetch + \texttt{toProductDetail} (done) (save-first noted in the editor flow;
        quick-view wrapper closes via \texttt{history.back()}). Duplicate semantics per spec (draft,
        \texttt{-copy} retry via unique-violation code 23505, featured off, end of list, features + links
        copied, image URLs by reference) (done). Categories add/rename/delete-guarded/reorder (done)
        (guard is app-level --- the FK is ON DELETE SET NULL, so the DB alone wouldn't block). Uploads to
        \texttt{product-images} via the server client, \texttt{image/*}, $\leq$ 5\,MB (done).
  \item \textbf{Global constraints honored:} only @dnd-kit added; public components/queries untouched
        (preview imports them); every mutation revalidates \texttt{/}, \texttt{/products}, detail path(s),
        \texttt{/admin/products}; no \texttt{service\_role}; Conventional Commits; green bar closes every
        task.
  \item \textbf{Task-boundary compile check:} Task 1 keeps \texttt{getProductAdmin} + the legacy
        form/toggle actions so the untouched pages still build; Task 2 removes the form-era exports with
        the pages that used them; Task 3 removes \texttt{toggleProductPublished} with the list rewrite
        (it bypassed publish validation --- spec \S3 forbids that path anyway). Each task ends green in
        isolation.
  \item \textbf{Placeholder scan:} every code step is complete file content or an exact named deletion ---
        no ``similar to'', no ``add validation here''.
  \item \textbf{Type consistency:} \texttt{AdminProductFull} is structurally accepted by
        \texttt{toProduct}/\texttt{toProductDetail} (extra \texttt{blog\_post\_id}/\texttt{status} join
        fields are ignored); \texttt{ProductDetail extends Product} feeds both preview tiers from one
        mapping call; \texttt{saveProductFull}'s discriminated \texttt{SaveProductResult} drives the
        editor's banner + escape hatch; keyed client rows (\texttt{HighlightItem}/\texttt{FeatureItem})
        exist only in the editor --- payload strips keys.
  \item \textbf{Known judgment calls (flagged for review):} duplicate also suffixes the \textit{name} with
        `` (copy)'' so the two rows are distinguishable in the list (spec only mandates the slug suffix);
        after saving a \textbf{new} product the editor navigates to \texttt{/admin/products/[id]} (so
        Preview/Publish are one click away) while existing products return to the list;
        \texttt{next.config.ts} uses \texttt{experimental.serverActions.bodySizeLimit} with an explicit
        fallback note if Next 16 has promoted the key; gallery images upload/remove without drag-ordering
        (spec lists drag only for highlights, features, related posts, list rows, and categories).
\end{itemize}
